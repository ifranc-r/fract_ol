...
void	menu_charac(t_all *all)
{
	...
	mlx_string_put(all->mlx, all->win, 3, 140, 0xFFFFFF, "z img:");
	mlx_string_put(all->mlx, all->win, 3, 160, 0xFFFFFF, "z real:");
	mlx_string_put(all->mlx, all->win, 3, 180, 0xFFFFFF, "x1:");
	mlx_string_put(all->mlx, all->win, 3, 200, 0xFFFFFF, "y1:");
	mlx_string_put(all->mlx, all->win, 3, 220, 0xFFFFFF, "x:");
	mlx_string_put(all->mlx, all->win, 3, 240, 0xFFFFFF, "y:");
}
...
{
	keycode == 123 ? all->coord.mousezx =\
	all->coord.x1 + (20 / all->coord.zoom) : 0;
	keycode == 124 ? all->coord.mousezx =\
	all->coord.x1 - (20 / all->coord.zoom) : 0;
	keycode == 126 ? all->coord.mousezy =\
	all->coord.y1 + (20 / all->coord.zoom) : 0;
	keycode == 125 ? all->coord.mousezy =\
	all->coord.y1 - (20 / all->coord.zoom) : 0;
}
